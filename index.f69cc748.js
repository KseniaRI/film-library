 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */})),vl.addEventListener("click",(async function(){const e=gl.value,t=_l.value;try{const n=await Tn(Kc,e,t);bl.textContent=`You've created account with email ${n.user.email}`}catch(e){Vc.Notify.failure(`${e.message}`)}})),wl.addEventListener("click",(async function(){await(e=Kc,J(e).signOut());var e})),Il.addEventListener("click",Vl),Tl.addEventListener("click",Vl),hl.addEventListener("click",(function(){if(Ul(),Number(kl.textContent)%5==0){const e=Number(kl.textContent)+1;return dl.innerHTML="",Ll(e,5),void(!1===Nl?Dl(e):Ml(e))}kl=kl.nextElementSibling,kl.classList.add("pagination__activ"),kl.previousElementSibling.classList.remove("pagination__activ");const e=kl.textContent;!1===Nl?Dl(e):Ml(e),Number(kl.textContent)===Sl?hl.disabled=!0:1===Number(kl.textContent)?ul.disabled=!0:ul.disabled=!1})),ul.addEventListener("click",(function(){if(Ul(),(Number(kl.textContent)-1)%5==0){const e=Number(kl.textContent)-1;dl.innerHTML="";return Ll(e-5+1,5),!1===Nl?Dl(e):Ml(e),kl=dl.lastElementChild,dl.firstElementChild.classList.remove("pagination__activ"),void kl.classList.add("pagination__activ")}kl.classList.remove("pagination__activ"),kl=kl.previousElementSibling,kl.classList.add("pagination__activ");const e=kl.textContent;!1===Nl?Dl(e):Ml(e),1===Number(kl.textContent)?ul.disabled=!0:ul.disabled=!1})),dl.addEventListener("click",(function(e){if(Ul(),kl.classList.remove("pagination__activ"),"BUTTON"!==e.target.nodeName)return;kl=e.target,kl.classList.add("pagination__activ");const t=kl.textContent;!1===Nl?Dl(t):Ml(t),1===Number(kl.textContent)?ul.disabled=!0:ul.disabled=!1})),nl.parentElement.classList.add("nav__item--active"),sl.classList.add("hidden"),Jc.classList.add("home-grid"),ql(),Jc.addEventListener("click",(async function(e){if("IMG"!==e.target.nodeName)return;const t=e.target.id,n=await Gc.fetchMovieInfo(t);al.classList.remove("is-hidden"),function(e){ol.innerHTML="";const t=function(e){const t=e.genres.map((e=>e.name)).join(", "),n=Number(e.popularity).toFixed(1).toString();return`\n      <img class="movie-card__poster" src="https://image.tmdb.org/t/p/w500${e.poster_path}" id = "${e.id}" alt="${e.title}"/>\n      <div class="film-card__info">\n            <h2 class="film-card__title">${e.title}</h2>\n            <div class="film-card__details">\n                  <p>\n                        <span class="film-card__detail">Vote / Votes</span>\n                        <b class = "film-card__value"><span class="film-card__value--accent">${e.vote_average}</span>/${e.vote_count}</b>\n                   </p>\n                  <p>\n                        <span class="film-card__detail">Popularity</span>\n                        <b class = "film-card__value">${n}</b>\n                  </p>\n                  <p>\n                      <span class="film-card__detail">Original Title</span>\n                      <b class = "film-card__value film-card__value--upper">${e.original_title}</b>\n                  </p>\n                  <p>\n                      <span class="film-card__detail">Genre</span>\n                      <b class = "film-card__value">${t}</b>\n                  </p>\n            </div>\n            <h3 class="film-card__about">About</h3>\n            <p class="film-card__description">${e.overview}</p>\n            <ul class="modal__option-buttons">\n                  <button type="button" class="btn-modal" id="watched">add to Watched</button>\n                  <button type="button" class="btn-modal" id="queue">add to queue</button>\n            </ul>\n      </div>`}(e);ol.insertAdjacentHTML("beforeend",t);const n=document.getElementById("watched"),i=document.getElementById("queue");jl=e.id,Wl=e,n.addEventListener("click",zl),i.addEventListener("click",zl)}(n),document.addEventListener("keydown",Pl),document.addEventListener("click",Al)})),cl.addEventListener("click",(function(){al.classList.toggle("is-hidden")})),ll.addEventListener("click",(function(){pl.classList.toggle("is-hidden")}));let kl=null,xl=[],Sl=0;let Nl=!1,Rl=null;function Pl(e){"Escape"===e.code&&al.classList.add("is-hidden"),document.removeEventListener("keydown",Pl)}function Al(e){console.log(e.target),e.target.classList.contains("backdrop")&&al.classList.add("is-hidden")}async function Ol(){var e,t,n;e=e=>{e?(bl.textContent=`You've entered with email ${e.email}`,ml.classList.add("hidden"),Cl.classList.remove("hidden"),ll.classList.remove("hidden"),Rl=e.uid,console.log(Rl)):(ml.classList.remove("hidden"),Cl.classList.add("hidden"),ll.classList.add("hidden"))},J(Kc).onAuthStateChanged(e,t,n)}function Ll(e,t){const n=function(e,t){let n=[];for(let i=e;i<=t;i+=1)n.push(`<button type="button" class="pagination__btn page">${i}</button>`);return n.join("")}(e,e+t-1);dl.insertAdjacentHTML("beforeend",n),xl=document.querySelectorAll(".page"),kl=xl[0],kl.classList.add("pagination__activ")}async function Dl(e){Qc.innerHTML="";const{results:t,total_pages:n}=await Gc.fetchTrendingMovies(e);Fl(),Bl(t,Qc)}async function Ml(e){Qc.innerHTML="";const{results:t,total_pages:n}=await Gc.fetchMoviesByKeyWord(e);Fl(),Bl(t,Qc)}function Ul(){document.querySelector(".spinner").classList.remove("hidden")}function Fl(){document.querySelector(".spinner").classList.add("hidden")}async function ql(){pl.classList.remove("is-hidden"),ll.classList.add("hidden"),Ol(),Fl();try{const{results:e,total_pages:t}=await Gc.fetchTrendingMovies(1);Bl(e,Qc),Sl=t,fl.classList.remove("hidden"),Ll(1,5),Nl=!1}catch(e){console.log(e)}}function Bl(e,t){const n=function(e){return e.map((e=>{let t="";var n;e.genre_ids?(n=e.genre_ids,t=El.filter((e=>n.includes(e.id))).map((e=>e.name)).join(", ")):t=e.genres.map((e=>e.name)).join(", ");const i=function(e){let t;return t=e?e.slice(0,4):"unknown year",t}(e.release_date);return`\n               <a class="film">\n                  <img class="film__poster" src="https://image.tmdb.org/t/p/w500${e.poster_path}" id = "${e.id}" alt="${e.title}" loading="lazy" />\n                  <div class="film__info">\n                        <p class="film__title">${e.title}</p>\n                        <p class="film__genre">${t}<span> | ${i}</span></p>\n                  </div>\n               </a>`})).join("")}(e);t.insertAdjacentHTML("beforeend",n)}let jl=null,Wl=null;function zl(e){const t=e.currentTarget.id;Oc(Rc(Nc(Wc()),`users/${Rl}/${t}`)).then((e=>{if(e.exists()){const n=e.val();Object.values(n).map((e=>e.film)).map((e=>e.id)).includes(Wl.id)||Hl(Rl,Wl,`/${t}`)}else Hl(Rl,Wl,`/${t}`)})).catch((e=>{console.error(e)}))}function Hl(e,t,n){Pc(Nc(Yc,"users/"+e+`${n}`),{film:t})}function Vl(e){let t=null;e.currentTarget.classList.contains("btn-watched")&&(t=Nc(Yc,"users/"+Rl+"/watched")),e.currentTarget.classList.contains("btn-queue")&&(t=Nc(Yc,"users/"+Rl+"/queue")),Uc(t,(e=>{const t=e.val(),n=Object.values(t).map((e=>e.film)),i=document.querySelector(".library-grid");i.innerHTML="",Bl(n,i),fl.classList.add("hidden")}))}
//# sourceMappingURL=index.f69cc748.js.map
