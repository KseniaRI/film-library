 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */})),yl.addEventListener("click",(async function(){const e=ml.value,t=gl.value;try{const n=await Tn(Kc,e,t);wl.textContent=`You've created account with email ${n.user.email}`}catch(e){Vc.Notify.failure(`${e.message}`)}})),vl.addEventListener("click",(async function(){await(e=Kc,J(e).signOut());var e})),Cl.addEventListener("click",Wl),Il.addEventListener("click",Wl),hl.addEventListener("click",(function(){if(Ll(),Number(El.textContent)%5==0){const e=Number(El.textContent)+1;return dl.innerHTML="",Pl(e,5),void(!1===Sl?Al(e):Ol(e))}El=El.nextElementSibling,El.classList.add("pagination__activ"),El.previousElementSibling.classList.remove("pagination__activ");const e=El.textContent;!1===Sl?Al(e):Ol(e),Number(El.textContent)===xl?hl.disabled=!0:1===Number(El.textContent)?ul.disabled=!0:ul.disabled=!1})),ul.addEventListener("click",(function(){if(Ll(),(Number(El.textContent)-1)%5==0){const e=Number(El.textContent)-1;dl.innerHTML="";return Pl(e-5+1,5),!1===Sl?Al(e):Ol(e),El=dl.lastElementChild,dl.firstElementChild.classList.remove("pagination__activ"),void El.classList.add("pagination__activ")}El.classList.remove("pagination__activ"),El=El.previousElementSibling,El.classList.add("pagination__activ");const e=El.textContent;!1===Sl?Al(e):Ol(e),1===Number(El.textContent)?ul.disabled=!0:ul.disabled=!1})),dl.addEventListener("click",(function(e){if(Ll(),El.classList.remove("pagination__activ"),"BUTTON"!==e.target.nodeName)return;El=e.target,El.classList.add("pagination__activ");const t=El.textContent;!1===Sl?Al(t):Ol(t),1===Number(El.textContent)?ul.disabled=!0:ul.disabled=!1})),nl.parentElement.classList.add("nav__item--active"),sl.classList.add("hidden"),Jc.classList.add("home-grid"),Ml(),Jc.addEventListener("click",(async function(e){if("IMG"!==e.target.nodeName)return;const t=e.target.id,n=await Gc.fetchMovieInfo(t);al.classList.remove("is-hidden"),function(e){ol.innerHTML="";const t=function(e){const t=e.genres.map((e=>e.name)).join(", "),n=Number(e.popularity).toFixed(1).toString();return`\n      <img class="movie-card__poster" src="https://image.tmdb.org/t/p/w500${e.poster_path}" id = "${e.id}" alt="${e.title}"/>\n      <div class="film-card__info">\n            <h2 class="film-card__title">${e.title}</h2>\n            <div class="film-card__details">\n                  <p>\n                        <span class="film-card__detail">Vote / Votes</span>\n                        <b class = "film-card__value"><span class="film-card__value--accent">${e.vote_average}</span>/${e.vote_count}</b>\n                   </p>\n                  <p>\n                        <span class="film-card__detail">Popularity</span>\n                        <b class = "film-card__value">${n}</b>\n                  </p>\n                  <p>\n                      <span class="film-card__detail">Original Title</span>\n                      <b class = "film-card__value film-card__value--upper">${e.original_title}</b>\n                  </p>\n                  <p>\n                      <span class="film-card__detail">Genre</span>\n                      <b class = "film-card__value">${t}</b>\n                  </p>\n            </div>\n            <h3 class="film-card__about">About</h3>\n            <p class="film-card__description">${e.overview}</p>\n            <ul class="modal__option-buttons">\n                  <button type="button" class="btn-modal" id="watched">add to Watched</button>\n                  <button type="button" class="btn-modal" id="queue">add to queue</button>\n            </ul>\n      </div>`}(e);ol.insertAdjacentHTML("beforeend",t);const n=document.getElementById("watched"),i=document.getElementById("queue");Fl=e.id,ql=e,n.addEventListener("click",Bl),i.addEventListener("click",Bl)}(n)})),cl.addEventListener("click",(function(){al.classList.toggle("is-hidden")})),ll.addEventListener("click",(function(){fl.classList.toggle("is-hidden")}));let El=null,kl=[],xl=0;let Sl=!1,Nl=null;async function Rl(){var e,t,n;e=e=>{e?(wl.textContent=`You've entered with email ${e.email}`,pl.classList.add("hidden"),bl.classList.remove("hidden"),ll.classList.remove("hidden"),Nl=e.uid,console.log(Nl)):(pl.classList.remove("hidden"),bl.classList.add("hidden"),ll.classList.add("hidden"))},J(Kc).onAuthStateChanged(e,t,n)}function Pl(e,t){const n=function(e,t){let n=[];for(let i=e;i<=t;i+=1)n.push(`<button type="button" class="pagination__btn page">${i}</button>`);return n.join("")}(e,e+t-1);dl.insertAdjacentHTML("beforeend",n),kl=document.querySelectorAll(".page"),El=kl[0],El.classList.add("pagination__activ")}async function Al(e){Qc.innerHTML="";const{results:t,total_pages:n}=await Gc.fetchTrendingMovies(e);Dl(),Ul(t,Qc)}async function Ol(e){Qc.innerHTML="";const{results:t,total_pages:n}=await Gc.fetchMoviesByKeyWord(e);Dl(),Ul(t,Qc)}function Ll(){document.querySelector(".spinner").classList.remove("hidden")}function Dl(){document.querySelector(".spinner").classList.add("hidden")}async function Ml(){fl.classList.remove("is-hidden"),ll.classList.add("hidden"),Rl(),Dl();try{const{results:e,total_pages:t}=await Gc.fetchTrendingMovies(1);Ul(e,Qc),xl=t,Pl(1,5),Sl=!1}catch(e){console.log(e)}}function Ul(e,t){const n=function(e){return e.map((e=>{let t="";var n;e.genre_ids?(n=e.genre_ids,t=Tl.filter((e=>n.includes(e.id))).map((e=>e.name)).join(", ")):t=e.genres.map((e=>e.name)).join(", ");const i=function(e){let t;return t=e?e.slice(0,4):"unknown year",t}(e.release_date);return`\n               <a class="film">\n                  <img class="film__poster" src="https://image.tmdb.org/t/p/w500${e.poster_path}" id = "${e.id}" alt="${e.title}" loading="lazy" />\n                  <div class="film__info">\n                        <p class="film__title">${e.title}</p>\n                        <p class="film__genre">${t}<span> | ${i}</span></p>\n                  </div>\n               </a>`})).join("")}(e);t.insertAdjacentHTML("beforeend",n)}let Fl=null,ql=null;function Bl(e){const t=e.currentTarget.id;Oc(Rc(Nc(Wc()),`users/${Nl}/${t}`)).then((e=>{if(e.exists()){const n=e.val();Object.values(n).map((e=>e.film)).map((e=>e.id)).includes(ql.id)||jl(Nl,ql,`/${t}`)}else jl(Nl,ql,`/${t}`)})).catch((e=>{console.error(e)}))}function jl(e,t,n){Pc(Nc(Yc,"users/"+e+`${n}`),{film:t})}function Wl(e){let t=null;e.currentTarget.classList.contains("btn-watched")&&(t=Nc(Yc,"users/"+Nl+"/watched")),e.currentTarget.classList.contains("btn-queue")&&(t=Nc(Yc,"users/"+Nl+"/queue")),Uc(t,(e=>{const t=e.val(),n=Object.values(t).map((e=>e.film)),i=document.querySelector(".library-grid");i.innerHTML="",Ul(n,i)}))}
//# sourceMappingURL=index.8a332e72.js.map
